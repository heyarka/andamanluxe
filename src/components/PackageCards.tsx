...
const PackageCards = ({ tripType, profile = "Indian Resident", onStartOver }: PackageCardsProps) => {
  ...
  const handleViewDetailsClick = (pkg: typeof packages[0]) => {
    ...
    <div>
      ...
      <div className="mb-6 md:mb-12 overflow-hidden">
        <HorizontalScroll className="md:grid-cols-3 gap-2" showEdgeFade={false} mobilePeek initialCenterIndex={defaultFocusedIndex} focusedCardIndex={focusedIdx} provokeSwipe>
          {packages.map((pkg, i) => {
            ...
            >
              {pkg.recommended && (
                <div className="bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 text-slate-950 text-center py-1.5 md:py-2 text-[10px] md:text-sm font-bold tracking-wide flex items-center justify-center gap-1.5">
              ...
              <div className="relative h-32 md:h-48 overflow-hidden">
                ...
                <span className="absolute bottom-2 right-2 rounded-full bg-white/90 backdrop-blur px-2.5 py-1 text-[9px] md:text-[10px] font-semibold text-slate-700 border border-white/70">
                  Signature Itinerary
                </span>
              </div>

              <div className="p-4 md:p-6">
                <h3 className="font-display text-sm md:text-[1.85rem] font-bold text-slate-900 mb-2 md:mb-3 leading-tight">{pkg.name}</h3>
                <div className="flex flex-wrap items-center gap-2 text-[10px] md:text-sm mb-2 md:mb-4">
                  <span className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-slate-700 font-medium">
                    <Clock className="h-3 w-3 md:h-4 md:w-4 text-amber-600" /> {pkg.duration}
                  </span>
                  <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-slate-700 font-medium">
                    <MapPin className="h-3 w-3 md:h-4 md:w-4 text-slate-500" /> {pkg.islands}
                  </span>
                </div>

                <p className="mb-2 md:mb-4">
                  <span className="text-lg md:text-2xl font-bold text-amber-600">{getPrice(pkg)}</span>
                  <span className="text-[10px] md:text-sm text-slate-500"> per person</span>
                </p>

                <ul className="space-y-1 md:space-y-2 mb-4 md:mb-6">
                  {pkg.features.map((f) => (
                    <li key={f} className="flex items-start gap-1.5 md:gap-2 text-[10px] md:text-sm text-slate-600">
                      <CheckCircle2 className="h-3 w-3 md:h-4 md:w-4 text-amber-500 mt-0.5 shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
                ...
                <div className="space-y-3">
                  <button
                    onClick={() => handleViewDetailsClick(pkg)}
                    className="w-full rounded-lg py-2 md:py-3 text-[10px] md:text-sm font-semibold border border-slate-200 text-slate-800 hover:bg-slate-50 transition-all"
                  >
                    View Details
                  </button>
                </div>
      ...
      <div className="glass-card rounded-xl md:rounded-2xl p-4 md:p-8 text-center">
        ...
        <div className="flex items-center justify-center gap-2 md:gap-4">
          <button onClick={() => { const message = `Hi, I need help choosing the right Andaman package. Can you assist?`; window.location.href = `https://wa.me/918637327297?text=${encodeURIComponent(message)}`; }} className="btn-frosted-blue rounded-lg px-4 py-2 md:px-8 md:py-3 text-[10px] md:text-sm font-semibold transition-all">Talk to an Expert</button>
          <button onClick={() => navigate("/customize")} className="rounded-lg border border-foreground/10 text-foreground px-4 py-2 md:px-8 md:py-3 text-[10px] md:text-sm font-semibold hover:bg-muted/50 transition-all">
            Customize
          </button>
        </div>
      </div>
      ...
      <PackageDetailsModal
        open={!!selectedDetailsPackage}
        selectedPackage={selectedDetailsPackage}
        isInternational={isInternational}
        onClose={() => {
          setSelectedDetailsPackage(null);
        }}
        onCustomize={(pkg, requestType) => {
          setSelectedCustomizationPackage(pkg);
          setCustomizationRequestType(requestType);
          setShowCustomizationPremium(true);
          setSelectedDetailsPackage(null);
        }}
      />

      <PackageCustomizationPremium
        open={showCustomizationPremium}
        selectedPackage={selectedCustomizationPackage}
        requestType={customizationRequestType}
        onClose={() => {
          setShowCustomizationPremium(false);
          setSelectedCustomizationPackage(null);
        }}
      />
    </div>
  );
